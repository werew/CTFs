...
static void aes128_key_schedule_inv_round(uint8_t* p_key, uint8_t rcon)
{
    uint_fast8_t    round;
...
void sub(uint8_t* p_key){
    int i;
    for (i=0; i<16; i++){
        p_key[i] = aes_sbox(p_key[i]);
    }
}

void sub_inv(uint8_t* p_key){
    int i;
    for (i=0; i<16; i++){
        p_key[i] = aes_sbox_inv(p_key[i]);
    }
}

void xor(uint8_t* v1, uint8_t* v2){
    int i;
    for (i=0; i<16; i++){
        v1[i] ^= v2[i];
    }
...
int main(){
    ...
    // Insert here the last round key to inserte
    uint8_t key[] = {0x4E,0x44,0xEA,0xCD,0x3F,0x54,0xF5,0xB5,0x4A,0x4F,0xB1,0x5E,0x07,0x10,0xB9,0x74};

    int start_round = 10;
   
    int i;
    for (i=start_round; i > 0; i--){
        aes128_key_schedule_inv_round(key,rcon[i-1]);
        print_hex(key);
    }
