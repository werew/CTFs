...
instr = {
         ...
         0x40:  "test",
         0x41:  "mov_reg",
         0x42:  "mov_data_to_reg",
         0x43:  "mov_data_from_reg",
         0x45:  "call",
         0x46:  "ret",
         0x47:  "jmpne",
         0x48:  "sub",
...
class CARM:
    ...
    def print_regs(self):
        ...
        print "edid:"+str(self.edid)
        print "esd:"+str(self.esd)
        print "edd:"+str(self.edd)
        print "esp:"+str(self.esp)
        print "espd:"+str(self.espd)
        print "eip:"+str(self.eip)
        print "eipd:"+str(self.eipd)
        print "ZF:"+str(self.ZF)
        print "CF:"+str(self.CF)
        print "SF:"+str(self.SF)
        print "OF:"+str(self.OF)
        print "NO:"+str(self.NO)

        for i in range(len(self.dataregs)):
            print "reg %d: " % i, self.dataregs[i]
    ...
    def a2r(addr):
        return CARM.REG(addr,addr)


    def command_disasm(self,opcode,addr):
        disasm_funs ={
             0x45:  self.__command_call,
        }
        disasm_funs[opcode](addr)


    def __command_call_disasm(self,addr):
        r = a2r(addr)
        d = self.eipd
        mrange.nextAddress(r,d) # nop
        reg=self.memory[self.__getMrange(addr)].getDWORD(self.eip,self.eipd)
        print "call ",reg
    ...
    def processComand(self):
        ...
        if opcode in self.commands.keys():
            ...
            print "\n############### STEP ###############"
            print "at: ", self.eip, self.eipd
            print "opcode: ",hex(opcode)
            print "amplifier: ",amplifier
            print instr[opcode]
            print
            #self.command_disasm(opcode,self.eip.real.value)
    ...
    def run(self):
        pwd=None
        while pwd!=True:
            if self.eip.imaginary.value != 0x60000:
                cmd = raw_input("werewdbg>")
            else:
                cmd = "s"
            ...
            if cmd == "s":
                ...
                try:
                    g = re.search(r"m *(0x[0-9a-f]*) *(0x[0-9a-f]*) *([0-9]*)",cmd)
                    addr = int(g.group(1),16)
                    addr2 = int(g.group(2),16)
                    steps = int(g.group(3))
                ...
                s = '' 
                for mrange in self.memory:
                    reg = CARM.REG(addr,addr2)

                    if mrange.containsAddr2(reg):
                        try:
                            s = mrange.readMemory(reg,self.eipd,steps)
                        except:
                            print "Access error"
                            s = ''
                if s == '':
                    ...
                    for r in range(len(s)//row_size + 1):
                        print "0x%x: " % (addr+r*row_size//2),
                        for i in range(row_size):
                            if r*row_size+i >= len(s): break
                            print "%02x " % ord(s[r*row_size+i]),
                        print
